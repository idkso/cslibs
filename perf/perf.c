...
static size_t perf_len(struct perf_ctx *ctx) {
	size_t ret = 0;
	ret += sizeof(uint64_t);
...
int perf_init(struct perf_ctx *ctx, uint8_t type, uint8_t types, uint8_t exclude) {
	int fd1 = -1, fd2;
...
#define SET(t) do { int fd, ret;										\
	...
	if (type == PERF_SW) pe.type = PERF_TYPE_SOFTWARE;
	
	if (BOOL(types & CPU_CYCLES)) SET(PERF_COUNT_HW_CPU_CYCLES);
	if (BOOL(types & INSTRUCTIONS)) SET(PERF_COUNT_HW_INSTRUCTIONS);
	if (BOOL(types & CACHE_REFERENCES)) SET(PERF_COUNT_HW_CACHE_REFERENCES);
	if (BOOL(types & CACHE_MISSES)) SET(PERF_COUNT_HW_CACHE_MISSES);
	if (BOOL(types & BRANCH_INSTRUCTIONS)) SET(PERF_COUNT_HW_BRANCH_INSTRUCTIONS);
	if (BOOL(types & BRANCH_MISSES)) SET(PERF_COUNT_HW_BRANCH_MISSES);
	if (BOOL(types & BUS_CYCLES)) SET(PERF_COUNT_HW_BUS_CYCLES);
...
int perf_read(int fd, struct perf_ctx *ctx, struct perf_res *res) {
	struct perf_buf buf;
	if (read(fd, &buf, perf_len(ctx)) == -1) return -1;
	for (uint64_t i = 0; i < buf.len; i++) {
		for (uint64_t j = 0; j < ctx->len; j++) {
			if (buf.res[i].id != ctx->ids[j]) continue;
